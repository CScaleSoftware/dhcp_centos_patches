...
Patch26:  %{name}-4.1.1-P1-logpid.patch
Patch27:  %{name}-4.1.1-P1-parse_date.patch
Patch28:  %{name}-4.1.1-P1-CVE-2010-3611.patch
Patch29:  %{name}-4.1.1-P1-rfc3442-classless-static-routes.patch
Patch30:  %{name}-4.1.1-P1-lpf-ib.patch
Patch31:  %{name}-4.1.1-P1-improved-xid.patch
Patch32:  %{name}-4.1.1-P1-gpxe-cid.patch
Patch33:  %{name}-4.1.1-P1-CVE-2011-0413.patch
Patch34:  %{name}-4.1.1-P1-confpars_ia.patch
Patch35:  %{name}-4.1.1-P1-CVE-2011-0997.patch
Patch36:  %{name}-4.1.1-P1-match_by_duid.patch
Patch38:  %{name}-4.1.1-P1-CVE-2011-2748-2749.patch
Patch41:  %{name}-4.1.1-P1-CVE-2011-4539.patch
Patch42:  %{name}-4.1.1-P1-parse_optional.patch
Patch43:  %{name}-4.1.1-P1-decline-onetry.patch
Patch44:  %{name}-4.1.1-P1-rfc5970-dhcpv6-options-for-network-boot.patch
Patch45:  %{name}-4.1.1-P1-CVE-2012-3571.patch
Patch46:  %{name}-4.1.1-P1-CVE-2012-3954.patch
Patch47:  %{name}-4.1.1-P1-getifaddrs.patch
Patch48:  %{name}-4.1.1-P1-CVE-2012-3955.patch
Patch49:  %{name}-4.1.1-P1-omapi-leak.patch
Patch50:  %{name}-4.1.1-P1-reply-to-source-port.patch
Patch51:  %{name}-4.1.1-P1-dupl-key.patch
Patch52:  %{name}-4.1.1-P1-range6.patch
Patch53:  %{name}-4.1.1-P1-do_relay4.patch
Patch54:  %{name}-4.1.1-P1-next-server.patch
Patch55:  %{name}-4.1.1-P1-multiple-6-clients.patch
Patch56:  %{name}-4.1.1-P1-rotate-v6-lease-file.patch
Patch57:  %{name}-4.1.1-P1-udp-cheksum-0xffff.patch
Patch58:  %{name}-4.1.1-P1-IPoIB-log-id.patch
Patch59:  %{name}-4.1.1-P1-paranoia.patch
Patch60:  %{name}-4.1.1-P1-relay6-pid-file.patch
Patch61:  %{name}-4.1.1-P1-option97-pxe-client-id.patch
Patch62:  %{name}-4.1.1-P1-vlanfilter.patch
Patch63:  add_vendor_encapsulated_options_if_not_in_prl.patch

Patch1000:  %{name}-4.1.1-P1-centos-branding.patch
...
# We use portreserve to prevent our ports being stolen.
...
# dhclient logs its pid to make troubleshooting systems
# with multiple dhclients running easier (#627257)
%patch26 -p1 -b .logpid

# Fix parsing of date (#631071)
%patch27 -p1 -b .parse_date

# Server Crash with Empty Link-Address Field (CVE-2010-3611)
%patch28 -p1 -b .CVE-2010-3611

# RFC 3442 - Classless Static Route Option for DHCPv4 (#558641)
%patch29 -p1 -b .rfc3442

# IPoIB support (#660681)
%patch30 -p1 -b .lpf-ib
%patch31 -p1 -b .improved-xid
%patch32 -p1 -b .gpxe-cid

# Unexpected abort caused by a DHCPv6 decline message (CVE-2011-0413, #672995)
%patch33 -p1 -b .CVE-2011-0413

# Write/accept a semi-colon at the end of the max-life and preferred-life
# clauses in dhcpv6 lease files (#672551)
%patch34 -p1 -b .confpars_ia

# dhclient: insufficient sanitization of certain DHCP response values
# CVE-2011-0997, #690579
%patch35 -p1 -b .CVE-2011-0997

# The 'hardware [ethernet|etc] ...;' parameter in host records has been
# extended to attempt to match DHCPv6 clients by the last octets
# of a DUID-LL or DUID-LLT provided by the client. (#706974)
%patch36 -p1 -b .match_by_duid

# A pair of defects cause the server to halt upon processing certain packets
# CVE-2011-2748, CVE-2011-2749, #729885
%patch38 -p1 -b .CVE-2011-2748-2749

# DoS due to processing certain regular expressions (CVE-2011-4539, #765683)
%patch41 -p1 -b .CVE-2011-4539

# Avoid infinite loop when parsing slp-service-scope option (#747017)
%patch42 -p1 -b .optional

# Send DHCPDECLINE and exit(2) when duplicate address was detected and
# dhclient had been started with '-1' (#756759).
%patch43 -p1 -b .decline-onetry

# RFC5970 - DHCPv6 Options for Network Boot (#798735)
%patch44 -p1 -b .rfc5970
...
# one client would receive the packets. (#1151054)
%patch55 -p1 -b .multiple-6-clients

# dhcpd: Rotate the lease file when running in v6 mode. (#1053431)
%patch56 -p1 -b .rotate-v6-lease-file

# dhcpd: Fix handling of UDP packets with checksum 0xffff. (#1015997)
%patch57 -p1 -b .udp-checksum-0xffff

# dhcpd: Add GUIDs in the DHCP logs for IPoIB. (#1064416)
%patch58 -p1 -b .IPoIB-log-id
...
%patch59 -p1 -b .paranoia

# Add --with-relay6-pid-file into configure script.
# Needed for adding dhcrelay6 init script (#1053155)
%patch60 -p1 -b .relay6-pid-file

# option 97 - pxe-client-id (#1058674)
# (Submitted to dhcp-bugs@isc.org - [ISC-Bugs #38110])
%patch61 -p1 -b .option97
...
for i in {client,relay,server,omapip,dhcpctl}/Makefile.am; do
        %{__sed} -i 's|fpie|fPIE|g' $i
done
for i in {common,omapip,dhcpctl,minires,dst}/Makefile.am; do
...
%configure \
    ...
    --with-relay6-pid-file=%{_localstatedir}/run/dhcrelay6.pid \
    --with-ldap \
    --with-ldapcrypto \
    --enable-paranoia
%{__make} %{?_smp_mflags}
...
%{__install} -p -m 0755 %{SOURCE9} %{buildroot}%{_initrddir}/dhcrelay6
...
%{__cat} << EOF > %{buildroot}%{_sysconfdir}/sysconfig/dhcrelay6
# Command line options here
DHCRELAYARGS=""
# Lower and upper interfaces for DHCPv6
LOWER_INTERFACES=""
UPPER_INTERFACES=""
EOF
...
# Tell portreserve which ports we want it to protect.
echo -e "dhcp-failover\ndhcp-failover2" > %{buildroot}%{_sysconfdir}/portreserve/dhcpd

%clean
%{__rm} -rf %{buildroot}

%pre
#uid:gid pair 177:177 reserved in setup (rhbz#738177)
getent group dhcpd >/dev/null || groupadd -f -g 177 -r dhcpd
getent passwd dhcpd >/dev/null || \
  useradd -r -u 177 -g dhcpd -d / -s /sbin/nologin -c "DHCP server" dhcpd
exit 0
...
if [ $1 = 0 ]; then
    ...
    /sbin/service dhcrelay6 stop >/dev/null 2>&1

    /sbin/chkconfig --del dhcpd
    /sbin/chkconfig --del dhcpd6
    /sbin/chkconfig --del dhcrelay || :
    /sbin/chkconfig --del dhcrelay6 || :
fi
...
if [ $1 -ge 1 ]; then
    ...
    /sbin/service dhcrelay6 condrestart >/dev/null 2>&1 || :
fi

%files
%defattr(-,root,root,-)
%doc ldap-for-dhcp-%{ldappatchver}/README.ldap
%doc dhcpd.conf.sample dhcpd6.conf.sample doc/IANA-arp-parameters doc/api+protocol
%doc __fedora_contrib/* ldap-for-dhcp-%{ldappatchver}/*.txt
...
%config(noreplace) %{_sysconfdir}/sysconfig/dhcrelay6
%config(noreplace) %{dhcpconfdir}/dhcpd.conf
%config(noreplace) %{dhcpconfdir}/dhcpd6.conf
%config(noreplace) %{_sysconfdir}/openldap/schema/dhcp.schema
%config(noreplace) %{_sysconfdir}/portreserve/dhcpd
%{_initrddir}/dhcpd
%{_initrddir}/dhcpd6
%{_initrddir}/dhcrelay
%{_initrddir}/dhcrelay6
%{_bindir}/omshell
%{_sbindir}/dhcpd
%{_sbindir}/dhcrelay
...
%changelog
* Wed Jul 22 2015 Johnny Hughes <johnny@centos.org> - 12:4.1.1-49.P1
- Roll in CentOS Branding

* Mon May 04 2015 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-49.P1
- Fix dhcrelay init script too. (#1187967)

* Wed Feb 25 2015 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-48.P1
- Actually take IB MAC address into account when creating seed (#1185075)

* Tue Feb 24 2015 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-47.P1
- Improve seeding the random number generator in dhclient (#1185075)

* Fri Feb 20 2015 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-46.P1
- Fix dhcrelay6 init script (#1187967)

* Wed Dec 17 2014 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-45.P1
- dhcrelay -6 was not working due to patch for #1001742 (#1151054)

* Wed Dec 17 2014 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-44.P1
- option 97 - pxe-client-id (#1058674)
...
* Tue Sep 25 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-34.P1
- Reducing the expiration time for an IPv6 lease may cause the server to crash
  (CVE-2012-3955, #858130)

* Thu Aug 23 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-33.P1
- Use getifaddrs() for interface discovery code on Linux (#803540)
- dhclient-script: do not backup&restore /etc/resolv.conf (#824622)

* Wed Jul 25 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-32.P1
- An error in the handling of malformed client identifiers can
  cause a denial-of-service condition in affected servers. (CVE-2012-3571, #843122)
- Memory Leaks Found In ISC DHCP (CVE-2012-3954, #843122)

* Wed May 09 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-31.P1
- don't flush old addresses in PREINIT (#815355)
- remove inherit-leases.patch - it's not needed anymore (#815355)

* Fri Mar 02 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-30.P1
- RFC5970 - DHCPv6 Options for Network Boot (#798735)

* Thu Feb 23 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-29.P1
- use portreserve (#790686)

* Tue Feb 14 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-28.P1
- -timeout option (command line) with value 3 or less was driving dhclient mad (#789719)

* Tue Feb 07 2012 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-27.P1
- dhclient-script: fix looking for other running dhclients (#656339)
- avoid infinite loop when parsing slp-service-scope option (#747017)
- dhclient-script: arping address in BOUND|RENEW|REBIND|REBOOT (#752116)
- send DHCPDECLINE and exit(2) when duplicate address was detected and
  dhclient had been started with '-1' (#756759).

* Fri Dec 09 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-26.P1
- DoS due to processing certain regular expressions (CVE-2011-4539, #765683)

* Tue Oct 18 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-25.P1
- change ownership of /var/lib/dhcpd/ to dhcpd:dhcpd (#693381)

* Thu Sep 15 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-24.P1
- Use statically allocated uid:gid 177:177 for user:group dhcpd (#693381)

* Tue Sep 13 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-23.P1
- Build libraries with -fpic/-fPIC, not with -fPIE (#736999)
- Add dhcp-common subpackage (#736194)
- Do not run dhcpd as root (#693381)
  - Use '--enable-paranoia' configure flag
  - Create dhcpd user in %%pre
  - Run dhcpd/dhcpd6 services with '-user dhcpd -group dhcpd'
  - Move changing of the effective user/group ID after writing new PID file.

* Fri Aug 19 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-22.P1
- Segfault when scanning TUN interfaces (#731990)

* Thu Aug 11 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-21.P1
- A pair of defects cause the server to halt upon processing certain packets
  (CVE-2011-2748, CVE-2011-2749, #729885)

* Thu Jun 16 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-20.P1
- The 'hardware [ethernet|etc] ...;' parameter in host records has been
  extended to attempt to match DHCPv6 clients by the last octets
  of a DUID-LL or DUID-LLT provided by the client. (#706974)
- Explicitly clear the ARP cache and flush all addresses & routes
  instead of bringing the interface down (#694799)
- Use PID for seeding the random number generator in dhclient (#694798)

* Thu Apr 07 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-19.P1
- Better fix for CVE-2011-0997: making domain-name check more lenient (#690579)

* Mon Apr 04 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-18.P1
- dhclient: insufficient sanitization of certain DHCP response values
  (CVE-2011-0997, #690579)

* Thu Mar 03 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-17.P1
- Fix dhclient-script to set interface MTU only when it's greater than 576 (#681721)

* Thu Jan 27 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-16.P1
- Unexpected abort caused by a DHCPv6 decline message (CVE-2011-0413, #672995)
- Write/accept a semi-colon at the end of the max-life and preferred-life
  clauses in dhcpv6 lease files (#672551)

* Tue Jan 25 2011 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-15.P1
- Add IPoIB support (#660681)

* Tue Dec 07 2010 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-14.P1
- RFC 3442 - Classless Static Route Option for DHCPv4 (#558641)

* Tue Nov 16 2010 Jiri Popelka <jpopelka@redhat.com> - 12:4.1.1-13.P1
- Add LICENSE file to dhclient subpackage (#613683)
- Add DHCRELAYARGS variable to /etc/sysconfig/dhcrelay (#625846)
- dhclient logs its pid to make troubleshooting systems
  with multiple dhclients running easier (#627257)
- Fix parsing of date (#631071)
